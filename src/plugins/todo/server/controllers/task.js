...
module.exports = createCoreController("plugin::todo.task", {
  ...
  async create(ctx) {
    try {
      ctx.body = await strapi.plugin("todo").service("task").create(ctx.request.body);
    } catch (err) {
      ctx.throw(500, err);
    }
  },
  async update(ctx) {
    try {
      ctx.body = await strapi.plugin("todo").service("task").update(ctx.params.id, ctx.request.body.data);
    } catch (err) {
      ctx.throw(500, err);
    }
  },
